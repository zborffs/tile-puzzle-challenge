...
%SOLVE solves the puzzle using your A* implementation
...
while i <= n-1 && j <= m-1
    if mod(i+j, 2) == 0 % lets complete a column (row-wise solve)
        i_bar = i;
        while i_bar <= n - 2
            board = put_tile(board, a(i_bar,j), Node(i_bar, j), solved_locs);
